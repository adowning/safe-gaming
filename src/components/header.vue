...
<template>
  <Popover class="relative bg-white">
    <div class="max-w-7xl mx-auto px-4 sm:px-6">
      <div class="flex justify-between items-center py-6 md:space-x-10">
        <div class="flex justify-start lg:w-0 lg:flex-1">
          <router-link to="/">
            <img class="h-8 sm:h-10 min-w-min" src="/assets/images/header-logo.svg" alt="" />
          </router-link>
        </div>
        <div class="-mr-2 -my-2 lg:hidden">
          <PopoverButton
            class="bg-white rounded-md p-2 inline-flex items-center justify-center text-voxo-gray hover:text-voxo-gray-dark hover:bg-gray-10 focus:outline-none ml-auto">
        ...
        <PopoverGroup as="nav" class="hidden lg:flex space-x-10">
          ...
          <router-link to="/product" class="text-base p-1 block"
            :class="$route.name === 'product' ? 'font-einasemibold text-voxo-gray-dark' : 'font-eina'">
            Product
          </router-link>

          <router-link to="/team" class="text-base p-1 block"
            :class="$route.name === 'team' ? 'font-einasemibold text-voxo-gray-dark' : 'font-eina'">
            Team
          </router-link>

          <Popover v-slot="{ open }" class="relative">
            <PopoverButton ref="flyoutMenu"
              :class="[open ? '' : '', 'group bg-white rounded-md inline-flex items-center text-base focus:outline-none font-eina p-1']">
              <span>Locations</span>
            ...
            <transition enter-active-class="transition ease-out duration-200" enter-from-class="opacity-0 translate-y-1"
              ...
              <PopoverPanel ref="mobileMenu"
                class="absolute z-30 left-1/2 transform -translate-x-1/2 mt-3 px-2 w-screen max-w-md sm:px-0 lg:max-w-3xl">
                <div class="rounded-lg shadow-lg ring-1 ring-black ring-opacity-5 overflow-hidden">
                  <div class="relative grid gap-6 bg-white px-5 py-6 sm:gap-8 sm:p-8 lg:grid-cols-2">
                    <router-link v-for="item in locations" :key="item.name" :to="`/locations${item.href}`"
                      class="-m-3 p-3 flex items-start rounded-lg hover:bg-gray-50 transition ease-in-out duration-150"
                      @click="hideFlyoutMenu">
                      <div
                        class="flex-shrink-0 flex items-center justify-center h-10 w-10 rounded-md text-white sm:h-12 sm:w-12">
          ...
          <router-link to="/contact" class="text-base p-1 block"
            :class="$route.name === 'contact' ? 'font-einasemibold text-voxo-gray-dark' : 'font-eina'">
            Contact
          </router-link>

          <router-link to="/blog" class="text-base p-1 block"
        ...
        <div class="hidden lg:flex items-center justify-end lg:flex-1 lg:w-0">
          <span class="inline-flex rounded-md shadow">
            <a target="_blank" href="https://app.voxo.co"
              class="inline-flex items-center px-4 py-2 border-2 border-voxo-red text-base font-medium rounded-lg text-voxo-red font-einasemibold">
              Log in
            </a>
          </span>
        </div>
    ...
    <transition enter-active-class="duration-200 ease-out" enter-from-class="opacity-0 scale-95"
      ...
      <PopoverPanel focus
        class="absolute top-0 left-1 inset-x-0 pl-2 pt-2 pr-5 transition transform origin-top-right lg:hidden z-30">
        <div class="rounded-lg shadow-lg ring-1 ring-black ring-opacity-5 bg-white divide-y-2 divide-gray-50">
          <div class="pt-5 pb-6 px-5">
            <div class="flex items-center justify-between">
              <div>
                <img class="h-8 w-auto" src="/assets/images/header-logo.svg" alt="VOXO" />
              </div>
              <div class="-mr-2">
                <PopoverButton ref="menuClose"
            ...
            <div class="mt-6">
              <nav class="grid gap-y-2">
                <router-link to="/" class="text-base p-1 block"
          ...
          <div class="py-6 px-5 space-y-6">
            <div class="grid grid-cols-2 gap-y-3 gap-x-8">
              <router-link v-for="item in locations" :key="item.name" :to="`/locations${item.href}`"
                class="text-base flex items-center" @click="hideMenu">
                <div
                  class="flex-shrink-0 flex items-center justify-center h-8 w-8 rounded-md text-white sm:h-12 sm:w-12">
                  <img :src="item.icon" :alt="item.name" />
                </div>
            ...
            <div>
              <a href="https://app.voxo.co" target=""
                class="w-full flex items-center justify-center px-4 py-2 border-2 border-voxo-red rounded-lg shadow-sm text-voxo-red font-einasemibold">
                Log in
              </a>
            </div>
          </div>
