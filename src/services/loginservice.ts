...
export async function login(
  password?: string,
): Promise<[LoginStatus, Device | null]> {
  ...
  const [httpStatus, userData] = await createDevice(
    encodeWebPushData(webPushData),
    password,
  )

  if (httpStatus === 401) return [LoginStatus.LOGIN_PASSWORD_REQUIRED, null]
...
export async function logoff(): Promise<LoginStatus> {
  ...
  const { id, secret } = ((await db
    .getAll('user')
    .then((users) => users && users[0])) ?? {}) as Device
