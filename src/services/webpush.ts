...
import type {
  ...
  ExtractedWebPushData,
} from '../types/webpushdata'
import { getWebPushData } from '../util/webpushutil'

async function getWebPushManger() {
  const sw = await navigator.serviceWorker.ready
  if (!sw.pushManager) throw new Error('Your device does not support webpush')
...
export async function createSubscription(
  serverKey: string,
): Promise<ExtractedWebPushData> {
  const pushManager = await getWebPushManger()
  const subscription: ExtendedSubscription = await pushManager
    .subscribe({
