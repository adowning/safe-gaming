...
function SendMessageToPHP(ws, params, ab) {
  ...
  const gameURL = `${serverConfig.prefix + serverConfig.host}/game/${gameName}/server?&sessionId=${sessionId}`
  ...
  request(options, (err, res, body) => {
    ...
    const packet = require('./mod/packet.js')
    let responsePacket = new packet.OutcomingPacket()
    ...
    if (sAnswer.action == 'Act21') {
      ...
      const gambleWin = payload.serverResponse.totalWin
    ...
    if (sAnswer.action == 'Act19') {
      ...
      var payload = JSON.parse(sAnswer.serverResponse.payload)
      var reels = payload.serverResponse.reelsSymbols
      const wins = payload.serverResponse.spinWins
...
if (serverConfig.ssl) {
  ...
  const ca = fs.readFileSync('./ssl/intermediate.pem', 'utf8')
...
wss.on('connection', (ws) => {
  ...
  ws.on('message', (message) => {
    const messageView8 = new Int8Array(message)
